...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    private final Channel parent; // 父亲，比如serversocket
    private final Integer id;
    
    private final Unsafe unsafe;// 进行实际地IO操作
    private final DefaultChannelPipeline pipeline;// pipeline
    ...
    private volatile EventLoop eventLoop; // 用来监听处理IO事件
    ...
    protected AbstractChannel(Channel parent, Integer id) {
        ...
        unsafe = newUnsafe(); // 抽象方法，不同的Channel子类实现不同的Unsace
        pipeline = new DefaultChannelPipeline(this);// 很多操作都委托给了pipeline
        
        // 监听 关闭Future
        closeFuture().addListener(new ChannelFutureListener() {
            @Override
            // 与ChannelFuture绑定的IO操作完成时调用
            public void operationComplete(ChannelFuture future) {
                allChannels.remove(id());// 删除缓存
            }
        });
