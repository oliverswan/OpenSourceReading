...
final class DefaultChannelHandlerContext extends DefaultAttributeMap implements ChannelHandlerContext {
    ...
    private final Channel channel;// 只持有一个channel
    ...
    // 当两个handlers在不同的线程中执行，他们是挨在一起的。
    // 彼此的buffers可以被同时访问，这样会导致竞争。
    // 为了避免这种情况，我们懒的方式创建额外的线程安全buffer叫做bridge。
    // 这样两个handler通过bridges访问彼此的buffer
    // 写入到brgdge的内容，被flushBridge方法写入到实际的buffer
    final AtomicReference<MessageBridge> inMsgBridge;//可以用原子方式更新的对象引用
    ...
    DefaultChannelHandlerContext(
        ...
        if (executor != null) {
            // Pin one of the child executors once and remember it 
            // 同一个channel使用同一个executor来发布事件
