...
public abstract class AbstractByteBuf implements ByteBuf {
    ...
    private final SwappedByteBuf swappedBuf;// 一个转换顺序的buf
    private final ByteOrder order;// 存储顺序
    ...
    protected AbstractByteBuf(ByteOrder endianness) {
        ...
        order = endianness;
        swappedBuf = new SwappedByteBuf(this);// 本buf转换过顺序的
    }
    ...
    public ChannelBufType type() {
        return ChannelBufType.BYTE;// 字节，消息两种
    }
    ...
    public ByteBuf readBytes(int length) {
    	// 检查是否超过了可读字节，抛出RuntimeException
        checkReadableBytes(length);
        if (length == 0) {
            return Unpooled.EMPTY_BUFFER;// 返回一个空的
        }
        // 创建一个新的buf
        ByteBuf buf = factory().getBuffer(order(), length);
        // 将自身的内容从readerIndex开始写入新建的buffer
    ...
    public void writeBytes(byte[] src, int srcIndex, int length) {
    	// 从写的游标出，将src的srcIndex位置的内容写入length长度到自身
        setBytes(writerIndex, src, srcIndex, length);// 交给实际的buf来具体处理字节的写入
        writerIndex += length;
    }
