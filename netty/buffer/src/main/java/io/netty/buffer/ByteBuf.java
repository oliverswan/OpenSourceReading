 ...
 *		5>更快的性能
 *
 * 一个可随机或者顺序存取的单字节或多字节(8位的)。
 * 本接口为一个或多个原始字节数组（byte[]）或者NIO的ByteBuffer，提供一个抽象的视图。
 ...
 * <h3>创建一个 buffer</h3>
 *
 * 推荐使用Unpooled中的帮助方法来创建一个buffer，而不是调用实现者的构造函数。
 * {@link Unpooled} rather than calling an individual implementation's
 * constructor.
 *
 * <h3>随机存取indexing</h3>
 *
 * 就像普通的原始字节数组, ByteBuf使用
 * <a href="http://en.wikipedia.org/wiki/Zero-based_numbering">zero-based indexing</a>.
 * 
 * 第一个字节的序号永远是0，最后一个字节的序号永远是容量减去1.
 * 
 * It means the index of the first byte is always {@code 0} and the index of the last byte is
 * always {@link #capacity() capacity - 1}. 
 * 
 ...
 *     byte b = buffer.getByte(i);// 根据位置来获取
 ...
 * read bytes.  并将该指针增加读取到的位置。
 * 
 * 如果读操作的参数也是一个ByteBuf，并且没有指定目标位置，那么指定的buffer的readerIndex也同时增加
 * 
 ...
 * <p>
 * 
 * 如果没有足够的内容空间，会抛出IndexOutOfBoundsException
 * 默认分配，包装，或者复制的buffer的readerindex是0
 * 
 ...
 *
 * 就是已经度过的字节，起初这段大小是0，它的大小接近writerIndex，随着读取操作的执行。
 * 通过调用discardReadBytes方法可以取消已经读过的字节。来收回未使用的空间。
 * 
 ...
 * </pre>
 * 
 * 并不能保证调用discardReadBytes方法后，可写字节的内容，
 * Please note that there is no guarantee about the content of writable bytes
 * after calling {@link #discardReadBytes()}.  
 * 可写的字节不会被一定大多数情况下，甚至会被填充完全不同的数据。
 * 
 ...
 * 
 * 可以同时将readerIndex和writerIndex设置为0，通过调用clear方法。
 * 它不会清空buffer内容。只是清空指针。
 * 该方法的语义与ByteBuffer.clear()是不同的
 * 
 ...
 *
 * 不同的indexOf方法帮助你定位一个值的序号。
 * 
 * ByteBufIndexFinder可以进行复杂的动态的序列查询。
 * 或者简单的静态单字节查询。
 * 
 ...
 * <p>
 * 
 * 如果你要解码变量长度数据，比如 NUL-terminated字符串。你可以发现bytesBefore(byte)方法也很管用。
 * 
 ...
 * 每一个 buffer里面都有2个标记序号，一个用来保存reader，一个用来保存writer
 * 调用reset方法可以重置两个游标的位置。和InputStream的reset方法不同，没有readlimit。
 * 
 ...
 * 
 * 通过调用duplicate,slice方法来创建一个现有buffer的view或者slice。
 * 
 * You can create a view of an existing buffer by calling either
 * {@link #duplicate()}, {@link #slice()} or {@link #slice(int, int)}.
 * 
 * 返回导出的buffer有一个独立的reader,writer指针和marker序号。
 * 它共享其他内不数据表示，就像NIO buffer那样做的一样。
 * 
 ...
 * <h4>Byte array</h4>
 * 
 * 如果ByteBuf是基于字节数组，你可以对数组通过array方法进行存取。
 * 未了判断buffer是不是基于一个字节数组，可以调用hasArray方法进行判断。
 ...
 * <h4>NIO Buffers</h4>
 *
 * 如果一个ByteBuf可以转换为NIO ByteBuffer，可以通过调用nioBuffer方法。
